...
PODIUM_EXCLUDE_CLAUSE = f"user_id NOT IN ({','.join([repr(u) for u in PODIUM_EXCLUDED_USERS])})"
...
top_users = pd.read_sql(f"""
    ...
    FROM sensor_readings
    WHERE chunk_id LIKE 'passive_%%' AND {PODIUM_EXCLUDE_CLAUSE}
    GROUP BY user_id
...
if not daily_steps.empty and len(daily_steps) >= 3:
    ...
    with vitals_cols[0]:
        ...
        <div class="stat-box">
            ...
            <div class="stat-value">{median_val:,.0f}</div>
        </div>
        """, unsafe_allow_html=True)
    with vitals_cols[1]:
        st.markdown(f"""
    ...
    st.markdown("""
    <div class="empty-state">
        <div class="empty-title">Not enough step data</div>
        <div class="empty-desc">Need at least 3 days of step data to compute baselines</div>
    </div>
    """, unsafe_allow_html=True)
